...
export class Mover {
  ...
  tick(posOverride) {
    ...
    if (Date.now() - (this._hbAt ?? 0) > 60000) {
      this._hbAt = Date.now();
      try { console.error(`[mover-hb] dest=${this.dest ? this.dest.col + ',' + this.dest.row : 'null'} path=${this.path ? this.pathIdx + '/' + this.path.length : 'null'} fan=${this._fanIndex} stuck=${this.stuckTicks} sends=${this._sendCount ?? 0} drops=${this.session?.client?._droppedUserMoves ?? 0} gateAge=${Date.now() - (this._lastReportAt ?? 0)} cli=${this.session?.client ? this.session.client.state : 'noclient'} pacer=${this.session?.pacer ? 'Y' : 'n'}`); } catch {}
    }

...
export const MOVE_CAP_MS = 1000; // = MOVE_INTERVAL (move.c:57) — the official client's cadence law
  ...
  tick(posOverride) {
    ...
    // never authorise a position.
    {
      ...
      if (this.path && this.pathIdx < this.path.length) {
        ...
        const _ahead = this._routeAhead(_g, this._simX ?? myProtoX, this._simY ?? myProtoY, _r ? RUN_STRIDE_PROTO : WALK_STRIDE_PROTO);
    ...
    if (Date.now() - _recAt > 30000) {
      ...
      if (_srv.source !== 'none' && _geo) {
        try {
          ...
          } else if (ng.col !== _srv.col || ng.row !== _srv.row) {
